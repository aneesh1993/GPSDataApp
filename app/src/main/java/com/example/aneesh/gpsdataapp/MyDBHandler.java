...
public class MyDBHandler extends SQLiteOpenHelper{
    ...
    private static final String DATABASE_NAME = "GPSData12.db";
    ...
    public ArrayList<String> readDB(){
        ...
        while(!c.isAfterLast()){
            if(c.getString(c.getColumnIndex(COLUMN_DATE_TIME)) != null &&
                ...
                Log.i("DBRead", "in if 1");
                resultString += c.getString(c.getColumnIndex(COLUMN_DATE_TIME));
                resultString += "\t";
                //resultList.add(c.getString(c.getColumnIndex(COLUMN_DATE_TIME)));

                Log.i("DBRead", "in if 2");
                resultString += c.getString(c.getColumnIndex(COLUMN_LAT));
                resultString += "\t";
                //resultList.add(c.getString(c.getColumnIndex(COLUMN_LAT)));

                Log.i("DBRead", "in if 3");
